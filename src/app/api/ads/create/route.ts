          ...
          try {
            ...
            for (const goal of combo.goals) {
              ...
              const asRes = await fetch(`${FB}/${adAccountId}/adsets`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  name: `${campaignName} - Ad Set`, campaign_id: cmp.id,
                  daily_budget: Math.round(Number(dailyBudget) * 100),
                  bid_strategy: 'LOWEST_COST_WITHOUT_CAP',
                  start_time: startDate || new Date().toISOString(), end_time: endDate,
                  billing_event: 'IMPRESSIONS', optimization_goal: goal,
                  targeting, promoted_object: { page_id: pageId },
              ...
              for (const token of [pageToken, userToken]) {
                const adRes = await fetch(`${FB}/${adAccountId}/ads`, {
                  ...
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    name: `${campaignName} - Ad`, adset_id: as2.id,
